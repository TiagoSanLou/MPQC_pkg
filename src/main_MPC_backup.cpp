...
FILE * time_file;
FILE * actuator_control_file;
FILE * prediction_file;
FILE * attitude_file;
FILE * attitude_sp_file;
    ...
    while (ok())
    {
   	    ...
   	    if( current_state.mode != "OFFBOARD"){          
			...
			if( !current_state.armed && (Time::now() - last_request > Duration(5.0))){
				if( arming_client.call(arm_cmd) &&
					...
					//string  attitude_filename_str    =  "outputs/" + hour + "_" + minute + 
					//"_attitude.csv";
					//string  attitude_sp_filename_str =  "outputs/" + hour + "_" + minute + 
					//"_attitude_sp.csv";	
					
					//const char * actuator_filename = actuator_filename_str.c_str();
					//const char * attitude_filename = attitude_filename_str.c_str();
					//const char * attitude_sp_filename = attitude_sp_filename_str.c_str();			
					
					
					const char * time_filename = "/home/odroid/catkin_ws/src/mpc_quad_px4/outputs/time_vec_c.txt";					
					const char * attitude_filename = "/home/odroid/catkin_ws/src/mpc_quad_px4/outputs/attitude.txt";
					const char * prediction_filename = "/home/odroid/catkin_ws/src/mpc_quad_px4/outputs/prediction.txt";
					const char * attitude_sp_filename = "/home/odroid/catkin_ws/src/mpc_quad_px4/outputs/att_sp.txt";
					const char * actuator_filename = "/home/odroid/catkin_ws/src/mpc_quad_px4/outputs/commands.txt";
				    
					time_file              = fopen (time_filename,"w");
					actuator_control_file  = fopen (actuator_filename,"w");
					prediction_file        = fopen (prediction_filename,"w");					
					attitude_file          = fopen (attitude_filename,"w");
					attitude_sp_file 	   = fopen (attitude_sp_filename,"w");
		...
		if(mpc_switch != 1){
			...
			if(imu_msg_updated == 1){
				...
				/*
				 * STANDARD MPC MATRIX CONVERSION
				 * */
				////Converting 'MPC.H' to qpOASES H
				//real_t H[(N*nu)*(N*nu)];
				//for(int j=0; j < (N*nu)*(N*nu) ; j++){
					//int liNum,colNum;
					//liNum  = j/(N*nu);
					//colNum = j-j/(N*nu)*(N*nu);
					//H[j] = MPC.H(liNum,colNum);
				//}	
						
					////cout << "MPC.H = " << MPC.H << endl;				
					//// cout << "H = " << endl;
					//// for(int j=0; j < (N*nu)*(N*nu) ; j++){
						//// cout << H[j] << endl;	
					//// }
		
				//// Converting 'MPC.Aineq' to qpOASES A
				//real_t A[MPC.Aineq.rows()*MPC.Aineq.cols()];
				//for(int j=0; j < MPC.Aineq.rows()*MPC.Aineq.cols() ; j++){
					//int liNum,colNum;
					//liNum  = j/MPC.Aineq.cols();
					//colNum = j-j/MPC.Aineq.cols()*MPC.Aineq.cols();
					//A[j] = MPC.Aineq(liNum,colNum);
				//}
				
					////cout << "MPC.Aineq = " << MPC.Aineq << endl;				
					//// cout << "A = " << endl;
					//// for(int j=0; j < MPC.Aineq.rows()*MPC.Aineq.cols() ; j++){
						//// cout << A[j] << endl;	
					//// }
				
				//// Converting 'MPC.F' to qpOASES format G
				//real_t G[N*nu];
				...
				//}
					...
					////cout << "MPC.yref_pred = " << MPC.yref_pred << endl;
				...
				//// Converting 'MPC.Bineq' to qpOASES format ubA
				//real_t ubA[MPC.Bineq.rows()];
				//for(int j=0 ; j < MPC.Bineq.rows() ; j++){
					//ubA[j] = MPC.Bineq(j,0);
				//}	
				...
				//// Converting 'MPC.utildemin' to qpOASES format lb
				//real_t lb[MPC.utildemin.rows()];
				//for(int j=0 ; j < MPC.utildemin.rows() ; j++){
					//lb[j] = MPC.utildemin(j,0);
				//}
				
					////cout << "MPC.utildemin = " << MPC.utildemin << endl;				
					//// cout << "lb = " << endl;
					//// for(int j=0; j < MPC.utildemin.rows() ; j++){
						//// cout << lb[j] << endl;	
					//// }					
				
				//// Converting 'MPC.utildemax' to qpOASES format ub
				//real_t ub[MPC.utildemax.rows()];
				//for(int j=0 ; j < MPC.utildemax.rows() ; j++){
					//ub[j] = MPC.utildemax(j,0);
				//}		
		
					////cout << "MPC.utildemax = " << MPC.utildemax << endl;				
					//// cout << "ub = " << endl;
					//// for(int j=0; j < MPC.utildemax.rows() ; j++){
						//// cout << ub[j] << endl;	
					//// }
							
					////cout << "Força Interrupção ..." << endl;	
					////return 0; //////////////////////////////////////
				/*
				 * STANDARD MPC MATRIX CONVERSION END
				 * */
				...
				*/	
				...
				// Convert STANDARD Solution to Eigen format
				//for(int j=0 ; j< N*nu ; j++){
					//xSol(j,0) = xOpt[j];
				//}
		...
		//Publish to text filess				
				fprintf(time_file,"%i\t%f\t%f\n",i,dt,node_run_time);
				fprintf(actuator_control_file,"%f\t%f\t%f\t%f\n",roll_cmd,pitch_cmd,yaw_cmd,throttle_sp);
				fprintf(attitude_file,"%f\t%f\t%f\t%f\t%f\t%f\n",
					roll/pi*180,  roll_rate/pi*180,
					pitch/pi*180, pitch_rate/pi*180,
					yaw/pi*180,   yaw_rate/pi*180
				);
				fprintf(prediction_file,"%f\t%f\t%f\t%f\t%f\t%f\n",
					pred_stateVector(0,0),pred_stateVector(1,0),
					pred_stateVector(2,0),pred_stateVector(3,0),
					pred_stateVector(4,0),pred_stateVector(5,0)
				);
				fprintf(attitude_sp_file,"%f\t%f\t%f\n",roll_sp/pi*180,pitch_sp/pi*180,yaw_sp/pi*180);
...
void rc_in_cb(const mavros_msgs::RCIn::ConstPtr& msg){
	...
	throttle_sp = 0.9*map_torque(channel_1, 980, 2000, 0,1);
	roll_sp     = 0.4*map_torque(channel_2, 980, 2000,-1,1);	
	pitch_sp    = 0.4*map_torque(channel_3, 980, 2000,-1,1);
	yaw_sp      = 0.4*map_torque(channel_4, 980, 2000,-1,1);
